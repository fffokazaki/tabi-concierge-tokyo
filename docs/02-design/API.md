...
- **エリアだけで絞った候補を返す経路では、上記2つに加えて `other` の欠損を1件添える**（[Issue #50](https://github.com/fffokazaki/tabi-concierge-tokyo/issues/50)）。詳細は下記
...
| `query` | `gaps` |
| --- | --- |
| `"上野"` / `"上野・浅草"` | 添えない（エリアの一覧が答えそのもの。ノイズを足さない） |
