...
判定の順序に意味がある。エリア外を先に弾き、キーワードが当たれば候補を返す。**当たらなかったときに初めて**、ジャンル指定の飲食（粒度不足）→ 渋谷の観光データ欠損（未公開）→ 分類指定の空振り → エリアだけの絞り込み、の順に落とす。ジャンル判定をエリアのフォールバックより後ろに回すと、答えられない問い（ラーメン）にエリアのデータセット一覧を返して欠損が消える。最後のエリアだけの絞り込みは、**エリア名のほかに何か訊かれていた場合に欠損を1件添える**（下記「エリア・フォールバックに添える欠損」）。
...
- **エリアだけで絞った候補を返す経路では、`other` の欠損を1件添える**（[Issue #50](https://github.com/fffokazaki/tabi-concierge-tokyo/issues/50)）。上記2つと同時には起きない（どちらもこの経路より手前で `unanswered` として返るため、この経路の `gaps` は必ず1件）。詳細は下記
...
| `"上野・浅草"` | 添えない。ただし**複数エリアと「エリアだけの質問」を区別できていない**（[Issue #52](https://github.com/fffokazaki/tabi-concierge-tokyo/issues/52)） |
...
- **複数の代表エリアが書かれた場合は取り落とす**（[Issue #52](https://github.com/fffokazaki/tabi-concierge-tokyo/issues/52)・未修正）。`"上野・渋谷"` は残余が空になるので欠損を添えないが、候補は `area` の解決結果（上野）の分だけで、渋谷の要求はどこにも残らない。`"上野・浅草"` は両方とも台東区で候補が重なるため実害が出ず、**データセットが分かれている組み合わせでだけ症状が出る**
- **この手当てが効くのは「キーワードが1件も当たらなかった」経路だけ**（[Issue #53](https://github.com/fffokazaki/tabi-concierge-tokyo/issues/53)・未修正）。1件でも当たるとキーワード経路で `answered` になり、答えていない興味は残らない。`"ナイトライフ、上野で夜遊びしたい"` は銭湯のキーワード「夜」が「夜遊び」に部分一致するため `gaps` なしで銭湯を返す（渋谷で効くのは、渋谷を収録するのが都市公園1件だけで銭湯が候補集合から外れるためで、判定が正しいからではない）。**興味チップを複数選ぶほど何かが当たるので、実際に旅程が組み上がるケースほど発火しない**
