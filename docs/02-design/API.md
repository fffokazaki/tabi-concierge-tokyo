...
- **`gaps` に載るのは、実測で「答えられない」と言い切れる既知の欠損だけ**。ジャンル指定の飲食（粒度不足）／対象エリア外の地名（`out_of_area`）／渋谷の観光データ未公開（`data_not_published`）の3つで、いずれも `unanswered` として返すときと**同じ値**を返す。自然文を興味に分解することはしない（分解は Step 5 の LLM 側の仕事で、スタブに作り込むと二重になる）
- **すべて答えられない場合は従来どおり `status: "unanswered"`。** `answered` ＋ 全部 `gaps` にはしない（それでは「答えがある」と嘘になる）
- **`gaps` は optional で、無いときはキーごと省く。** 既存クライアントは無視しても壊れないが、無視すると欠損が画面に出ない点に注意
- キーワードが当たらずエリアだけで候補を返す経路でも `gaps` は載る
