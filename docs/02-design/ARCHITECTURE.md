...
3. **データセキュリティ**
   - 回答に使う事実データは公開済みオープンデータのみ。未回答ログの `question` は利用者の自由入力なので個人情報を含む可能性があり、認証のない `GET /api/gaps/summary` では個票を読まず集計値だけを公開する
...
- `gaps.question` は自由入力で個人情報を含む可能性があるため、集計 API が個票を読まず集計値だけを公開するセキュリティ境界を追記
