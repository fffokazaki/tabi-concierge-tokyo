...
旅コンシェルジュTOKYO は、訪日観光客向けの AI 旅行ガイドアプリ（フロントエンド）と、東京都オープンデータカタログ約9,600データセットから答えを組み立てる「オープンデータ・コンシェルジュ」（バックエンド）の2層で構成される。バックエンドは単一の Cloudflare Worker が **`/api/*`（React アプリ向け JSON）と `/mcp`（AI クライアント向け MCP）の二面**で同じコアロジックを公開する（ADR-008）。
