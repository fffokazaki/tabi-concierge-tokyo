...
| `status: "unanswered"` | 従来どおり見出し「該当するオープンデータがありません」＋ `message`。エラー表示にはしない。名乗りは見出しだけが持ち、`message` は事実だけを述べる（[Issue #107](https://github.com/fffokazaki/tabi-concierge-tokyo/issues/107) で `message` 側の名乗りを外した） |
