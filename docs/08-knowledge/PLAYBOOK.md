...
`check-category-size` がカテゴリ件数で止めた（または refine 目安の警告を出した）ときの手順（**本リポジトリでは CLI の入口を用意していないため、この発動はまだ起こらない**。§概要の導入状態の表を参照）:
...
- ACE-209-1: レビューの `--base` がローカル ref を見るため、diff が PR の範囲とずれる（PR #209）
- ACE-209-2: SPA の 200 はデプロイの証拠にならない。本番バンドルの固有文字列で裏を取る（PR #209）

#### カウンター更新

- ACE-202-1: Helpful +1（生成物側の SSOT バージョンが `deck/build.js` の未同期を実際に検出した → Issue #211）
- ACE-181-1: Helpful +1（README が縮退経路の署名 `固定データ抽出（スタブ）` を期待値として見せ続けていた）
...
- ACE-200-1: 集計軸が任意入力由来なら個票と同じ機密値として扱い、SQL 境界で公開分類へ丸める（Issue #193 / PR #200）
- ACE-200-2: プライバシー境界テストは機密値らしいセンチネルを実 DB へ入れ、集計整合と非露出を同時に固定する（Issue #193 / PR #200）
...
- ACE-190-1: 生成 SQL の実行結果も投影値は未信頼 — 安定 ID から固定 SQL で事実列を再取得する（Issue #152 / PR #190）

#### 変更

- ACE-137-2 / ACE-175-2: 生成 SQL が投影値を偽装できる反例が確認されたため deprecated。ACE-190-1 が置き換える
...
- ACE-181-1: Helpful +1（`aggregateDataset` の入口を主経路と縮退経路で書き分け、旧スタブ時代の保証を撤回）
- ACE-137-3: Helpful +1（`guardAggregate` を主経路と縮退先へ一元適用する現在の構造を、実在する関数名で説明）
- ACE-93-3: Helpful +1（別担当の `src/` に残る同型コメントを棚卸しし、Issue #186 へ切り分け）
- ACE-62-3: Helpful +1（縮退時の利用者向け `query` 文言を完全一致テストで固定）
- ACE-62-2: Helpful +1（旧関数名と誤った保証を横断検索し、`worker/` の取り残しを0件まで確認）
- ACE-181-1: Helpful +1（LLM 分解を主経路へ追加しても旧キーワード経路が縮退先に残るため、旧いスコアリング記述が部分的には正しく見えて残存した）
...
- §密度超過時の手順に、`check-category-size` の件数ゲートは**本リポジトリでは CLI の入口が無いためまだ発動しない**旨を追記した（PR #97）
