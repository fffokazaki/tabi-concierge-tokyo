...
| 上記以外（数値でない・範囲外・小数） | **既定の 3600 秒へ倒れ、`console.error` が出る**（`npx wrangler tail` で拾う）。Worker は落とさない — 収録直前に全体が死ぬほうが害が大きいため |
...
> **実測（2026-08-22・ローカル `npm run dev`・同一質問文を4回）**: 既定 `"3600"` では 1.78 → 1.00 → 0.98 → **0.48 秒**と落ちるのに対し、`"0"` では 1.57 → 1.73 → 1.24 → 1.66 秒で落ちない。ただし **`cf-aig-cache-status` はバインディング経由では読めない**ため、これは所要時間から見た**傍証**である。確証はダッシュボード（AI → AI Gateway → `default`）の Cache HIT / MISS で取ること。不正値（`AI_GATEWAY_CACHE_TTL=いちじかん`）では HTTP 200 のまま `console.error` が出て既定へ倒れることも同日に実測した。
