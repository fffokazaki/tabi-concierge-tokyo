...
> ⚠️ **SAMPLE — テンプレートです（本プロジェクト未適用）**
> 本ファイルの Q1〜Q6 の分岐内容（Web API バックエンドの例）と追加先チェックリストは **あなたのプロジェクトに合わせて書き換え** てください。
> 書き換え手順は本ファイル「4. プロジェクト固有化の手順」を参照。
>
> **本プロジェクトでの扱い（2026-08-15 時点）**: 実構成は `frontend/`（React UI）・`mcp-server/`（MCP ツール）・`data/`（同梱オープンデータ）であり、下記サンプルの REST / GraphQL / DB 前提の分岐とは一致しません。
> 実装着手までは、配置判断は [ARCHITECTURE.md](../02-design/ARCHITECTURE.md) §3 のコンポーネント設計と [MASTER.md](../MASTER.md)「ディレクトリ構造」を根拠にしてください。本ファイルを必須の判断根拠として参照しないこと。
> リンク先の `templates/` と `DEPENDENCY_LINT.md` は初期セット外のため本リポジトリに存在しません（必要時に ff-dev-toolkit の `docs-template/` から同一相対パスでコピー）。
