...
> 本文のコード例は NestJS・TypeORM・REST 前提の汎用テンプレートで、本プロジェクトの構成（React ＋ MCP ＋ Cloudflare・DB なし・認証なし）とは一致しません。
> [MASTER.md](../MASTER.md) のコード生成ルールが本ファイルを参照していますが、**現時点で本プロジェクトの確定事項として扱えるのは「マジックナンバー禁止」等の言語非依存の原則のみ**です。実装着手時に実態へ書き換えること。
