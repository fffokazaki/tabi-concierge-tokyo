...
- [x] **提出フォームに添付する画面キャプチャ 1600×900px を3点用意した**（2026-08-22・本番 `tokyo-odh-091`・興味チップのみ・自由文は空）。**資料に埋め込む 480×874 の端末フレーム画像とは別物**で、そちらを添付しても規定を満たさない（[CONSTRAINTS.md](01-context/CONSTRAINTS.md) §7 の変更不可の規定）。保管先は `~/Documents/tabi-concierge-tokyo-deck/submit/`。撮り方は [deck/README.md](../deck/README.md) に記録した
