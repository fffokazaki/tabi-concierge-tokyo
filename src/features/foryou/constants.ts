 ...
 * 「まだ覆えていない興味に一致する候補へ先に1枠ずつ充てる」ようになったため
 * （`worker/core/search-gaps.ts` の `selectWithInterestCoverage`・API.md §3.1
 * 「興味カバレッジ優先の選定」）、**4 に戻しても一致先を持つ興味は落ちない**。
 * 4 と 6 の違いは「同点の残り2件も見せるか」だけで、どちらもフロントエンド側の
 * 表示件数の選択（`search_datasets` の上限は10件・6 はその範囲内）。
 */
