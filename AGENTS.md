...
4. **カタログ掲載データ以外を組み込まない** — CC BY 4.0 のカタログ掲載データのみ二次利用可（[CONSTRAINTS.md](docs/01-context/CONSTRAINTS.md) §3）。**例外（[ADR-012](docs/06-reference/DECISIONS.md)）**: マナー欄の「参考」情報に限り、JNTO 等の外部情報を「出典」と表示しない形（見た目・ラベルを区別）で案内できる。停留地名・所在地など回答の事実データには適用しない
