...
>
> **構造化入力 `interests` を送る呼び出しでは、まだ覆えていない興味に一致する候補へ先に1枠ずつ充ててから残り枠をスコア順で埋める**（`selectWithInterestCoverage`・[Issue #84](https://github.com/fffokazaki/tabi-concierge-tokyo/issues/84) で修正済み）。枠の確保に使う述語は欠損の判定と同一なので、「枠に入らなかっただけなのに欠損に見える」は起きない。**ただし `no` 昇順そのものは残っている** — 残り枠を埋める段の同点解決は引き続き登録順で、自然文に畳み込んだ興味（プラン画面の `buildQuery`）にはこの手当ては効かない。スタブの既知の限界で、Step 5（Text-to-SQL 化）で見直す対象。詳細は [API.md](docs/02-design/API.md) §3.1「興味カバレッジ優先の選定」。
>
