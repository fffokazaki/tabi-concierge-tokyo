...
1. **推測で埋めない** — 仕様が不明な点は推論で補わず、ユーザーに確認する。文書に「未定」「未確認（実装着手時に確定）」と書かれている値を、それらしい数値で置き換えない
   - **否定・限定の主張は実行で裏を取る** — 「〜は起きない」「〜に限られる」と書く前に、それが起きる入力を実際に流す。ガードのコードを読んで正しく見えることと、出力で起きないことは別であり、実測か変異テストで確認する。目視レビューだけでは確認済みとしない。[PR #167](https://github.com/fffokazaki/tabi-concierge-tokyo/pull/167) では `countRelaxed` のガードを出力制約と読み違え、「すり替え先は同じエリアに限られる」「代表エリアを読み取れたときは限られる」という主張を同じ PR 内で2回実測に覆された
