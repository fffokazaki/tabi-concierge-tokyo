...
> （`src/` 70 ファイル・8,279 行のうち帰属できた 7,960 行。2026-09-19 実測）。
...
>
> ```bash
> git log --since=2026-08-15 --until=2026-08-24 -i --grep='Co-Authored-By: Claude' --oneline | wc -l
> ```
>
> **`-i` が要る。** このリポジトリには `Co-Authored-By:` と `Co-authored-by:` の2表記が混在していて、
> 大文字小文字を区別して数えると 58 件に見える（この README は最初その値を載せていた）。
