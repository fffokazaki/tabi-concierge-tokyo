...
> （`src/` のテキスト 61 ファイル・**7,960 行すべてを帰属できた**内訳。`src/` には他に `.webp` が 9 件あるが、
> 画像は行数に数えない。オプション無指定の `git blame`・2026-09-19 実測。`-w` や `-M -C` を付けると
> 按分が動くので、比較するときは同じオプションで測ること）。
...
**この表で一番言いたいのは PR 数やコミット数ではなく、上3行の比率のほう。**
...
文書を先に書くのは、その先で **AI が人の承認を待たずに最後まで走り切れるワークフロー**を成立させるため。
チェーンはこの形になる。
...
> **このリポジトリ自身は、上記を緩めた軽量フローで運用している。** PoC 段階のため Issue 起票と ACE を
> **任意**、レビューを「変更内容に応じて実施」としている（[ADR-006](docs/06-reference/DECISIONS.md)・
> [CLAUDE.md](CLAUDE.md) の Git Workflow）。上のチェーンは、メンテナ環境でゲートを全部有効にしたときの形。
