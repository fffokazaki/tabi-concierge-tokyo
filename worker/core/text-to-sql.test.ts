...
/**
 * ## エリアで絞るなら効かせる（Issue #174）
 *
 * `aggregate_dataset` の入力にエリアは無い（`datasetId` と `intent` だけ）。それでも LLM は
 * プロンプトが渡す「この dataset_id の行に実際に入っている値 ―― area: 「上野」「浅草」」を見て、
 * **自分の判断で** area 条件を足してくる。問題は繋ぎ方が2通りに割れることで、
 * `OR` で繋ぐと area 条件が実質効かず、**対象エリア外の行が category だけで当たる**。
 *
 * 本番実測（2026-08-22・Version `3093b95c`・キャッシュ無効）: 同一入力8回で `AND` 4回 /
 * `OR` 4回。`OR` 形は「文化、家族向け、自然」で蔵前の行（初代川柳墓）を旅程の先頭に出した。
 *
 * **エリアで絞ること自体は強制しない。** 絞らない判断はありうる（このデータセットが訊かれた
 * エリアを収録していないときの0行は正しい未回答・Issue #148）。禁じるのは「絞ったふりをして
 * 効かせない」形だけである。
 *
 * この検査は**プロンプト文字列に指示が載っていること**しか見ていない ―― 生成が指示に従うか
 * どうかは本番で引き直して測る（Issue #174 の AC）。プロンプトへ載せるところまでが単体で
 * 固定できる範囲である。
 */
describe("エリアで絞るなら AND で繋がせる（Issue #174）", () => {
  it("area を OR で繋がないようプロンプトで指示する", async () => {
    await seed([{ name: "寛永寺", area: "上野" }]);
    const llm = scriptedLlm(SELECT_ALL);

    await aggregate(MEISHO_ID, "文化、家族向け、自然", depsWith(llm));

    const prompt = llm.asked[0]!.user;
    expect(prompt, "OR で繋ぐ形を名指しで禁じる").toContain("OR で繋がない");
    // 「エリア外の行」だけでは素通りする ―― プロンプトには既に
    // 「対象エリア外の行は NULL」があり、この検査が何も見ていない状態になる
    expect(prompt, "禁じる理由（絞ったことにならない）を添える").toContain("絞ったことにならない");
  });

  it("エリアで絞らない選択肢は残す（絞りを強制すると偽の未回答が増える）", async () => {
    await seed([{ name: "寛永寺", area: "上野" }]);
    const llm = scriptedLlm(SELECT_ALL);
    ...
    expect(
      llm.asked[0]!.user,
      "「必ず area で絞れ」とは言わない ―― Issue #148 の逆方向（絞りすぎ）へ倒れる",
    ).not.toContain("area で必ず絞る");
  });
});
