...
const SELECT_ALL = `SELECT dataset_id, name, area, address, note FROM spots WHERE dataset_id = '${MEISHO_ID}'`;
  ...
  it("**別データセットの行が返ったら縮退する（生成 SQL では偽装できない検証）**", async () => {
    ...
    try {
      ...
      const llm = scriptedLlm(
        `SELECT dataset_id, name, area FROM spots WHERE dataset_id = '${MEISHO_ID}' OR 1=1 ORDER BY dataset_id`,
      );
...
describe("実在しない値では絞らせない（Issue #148）", () => {
  ...
  it("intent の語を name へ当てにいく SQL は0行になり、書き直させる（偽の未回答にしない）", async () => {
    ...
    try {
      ...
      const overRestrictive =
        `SELECT dataset_id, name, area, address, note FROM spots WHERE dataset_id = '${MEISHO_ID}'` +
  ...
  it("部分一致でしか当たらない値の**完全一致**は拒否する（Issue #120 で0行になった形）", async () => {
    ...
    try {
      ...
      const relaxed = `SELECT dataset_id, name, area, address, note FROM spots WHERE dataset_id = '${MEISHO_ID}' AND category LIKE '%文化%' LIMIT 50`;
  ...
  it("書き直しても実在しない値のままなら縮退する（偽の未回答にしない）", async () => {
    ...
    try {
      await seed(BOTH_AREAS);
      const invented = `SELECT dataset_id, name, area, address, note FROM spots WHERE dataset_id = '${MEISHO_ID}' AND category = 'ラーメン' LIMIT 50`;
  ...
  it("ダブルクォートの文字列でも偽の未回答にはならない", async () => {
    ...
    try {
      await seed(BOTH_AREAS);
      const doubleQuoted = `SELECT dataset_id, name, area, address, note FROM spots WHERE dataset_id = '${MEISHO_ID}' AND category = "自然" LIMIT 50`;
  ...
  it("要求を満たす行が無かった SQL は記録する（原因を後から分けるため）", async () => {
    ...
    try {
      ...
      const noRow = `SELECT dataset_id, name, area, address, note FROM spots WHERE dataset_id = '${MEISHO_ID}' AND area = '渋谷' LIMIT 50`;
  ...
  it("否定の演算子は拒否する（全行を除外して0行になる）", async () => {
    ...
    try {
      ...
      const negated = `SELECT dataset_id, name, area, address, note FROM spots WHERE dataset_id = '${MEISHO_ID}' AND category NOT IN ('名所・史跡') LIMIT 50`;
  ...
  it("dataset_id は `=` だけ（`!=` は別データセットへ広がる）", async () => {
    ...
    try {
      await seed(BOTH_AREAS);
      const negatedId = `SELECT dataset_id, name, area, address, note FROM spots WHERE dataset_id != '${MEISHO_ID}' LIMIT 50`;
  ...
  it("LIKE はワイルドカードの位置まで見る（`'文化'` と `'文化%'` は「区民文化財」に当たらない）", async () => {
    ...
    try {
      await seed([{ name: "旧朝倉家住宅", area: "渋谷", category: "区民文化財" }]);
      const prefixOnly = `SELECT dataset_id, name, area, address, note FROM spots WHERE dataset_id = '${MEISHO_ID}' AND category LIKE '文化%' LIMIT 50`;
      const ok = `SELECT dataset_id, name, area, address, note FROM spots WHERE dataset_id = '${MEISHO_ID}' AND category LIKE '区民%' LIMIT 50`;
  ...
  it("当たる LIKE は素通しする（`'%文化財'` は「区民文化財」に当たる）", async () => {
    await seed([{ name: "旧朝倉家住宅", area: "渋谷", category: "区民文化財" }]);
    const suffix = `SELECT dataset_id, name, area, address, note FROM spots WHERE dataset_id = '${MEISHO_ID}' AND category LIKE '%文化財' LIMIT 50`;
  ...
  it("`%` を大量に含むパターンでも即座に判定する（破滅的バックトラックを起こさない）", async () => {
    ...
    try {
      ...
      const evil = `SELECT dataset_id, name, area, address, note FROM spots WHERE dataset_id = '${MEISHO_ID}' AND category LIKE '${pathological}' LIMIT 50`;
...
describe("0行は緩めた問いでも0行のときだけ「無かった」と言う（Issue #148）", () => {
  it("リテラルを持たない絞り込み（`1 = 0`）で0行になっても、偽の未回答にはしない", async () => {
    ...
    try {
      ...
      const constantFalse = `SELECT dataset_id, name, area, address, note FROM spots WHERE dataset_id = '${MEISHO_ID}' AND 1 = 0 LIMIT 50`;
  ...
  it("本当に行が無いときは未回答のまま（正しい0行は壊さない）", async () => {
    ...
    try {
      await seed([{ name: "寛永寺", area: "上野" }]);
      const shibuya = `SELECT dataset_id, name, area, address, note FROM spots WHERE dataset_id = '${MEISHO_ID}' AND area = '渋谷' LIMIT 50`;
  ...
  it("書き直しても0行のままなら縮退する（未回答として名乗らない）", async () => {
    ...
    try {
      await seed([{ name: "寛永寺", area: "上野" }]);
      const llm = scriptedLlm(`SELECT dataset_id, name, area, address, note FROM spots WHERE dataset_id = '${MEISHO_ID}' AND 1 = 0 LIMIT 50`);
  ...
  it("施設名で1行を選ぶ SQL は通す（利用者が名指しした施設を別の行にすり替えない）", async () => {
    ...
    const byName = `SELECT dataset_id, name, area, address, note FROM spots WHERE dataset_id = '${MEISHO_ID}' AND name LIKE '%浅草寺%' LIMIT 50`;
...
describe("実行結果を要求エリアと照合する（Issue #152）", () => {
  const selectArea = (areaClause: string) =>
    `SELECT dataset_id, name, area, address, note FROM spots WHERE dataset_id = '${MEISHO_ID}' ${areaClause} LIMIT 50`;
  ...
  it("代表エリアを読み取れなくても area 列が無ければ書き直させる", async () => {
    await seed([{ name: "寛永寺", area: "上野" }]);
    const withoutArea = `SELECT dataset_id, name, address, note FROM spots WHERE dataset_id = '${MEISHO_ID}' LIMIT 50`;
