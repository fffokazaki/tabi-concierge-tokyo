 ...
 * 損失ではない。
 *
 * **すり替わる範囲は intent から代表エリアを読み取れたかで変わる。** 読み取れたときは
 * そのエリアの行に限られる（浅草を1行も収録していないデータセットで「浅草寺」を名指し
 * されても、上野の行を代わりに返したりはせず未回答になる）。**読み取れなかったときは
 * データセット全体から選ばれる** ―― `findRepresentativeArea` が拾うのは「上野」「浅草」
 * 「渋谷」の部分一致だけなので、未知の地名（巣鴨のとげぬき地蔵）やエリア無指定では
 * `areaClause` が空になり、エリアの歯止めは効かない。**「エリアは越えない」と書くと実装より
 * 強い主張になる**（cross-model レビュー・信頼度96。`text-to-sql.test.ts` の
 * 「代表エリアを読み取れない intent では、すり替え先はエリアを越える」が実測を固定している）。
 * 訊かれたエリアが生成 SQL から落ちる形は
 * [Issue #152](https://github.com/fffokazaki/tabi-concierge-tokyo/issues/152) が別に扱う。
 *
