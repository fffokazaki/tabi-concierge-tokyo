...
function buildPrompt(
  ...
  const lines = [
    ...
    // 入力にエリアは無い（`datasetId` と `intent` だけ）ので、LLM は上の「実際に入っている値」を
    // 見て自分で area 条件を足してくる。その繋ぎ方が `OR` に割れると条件が効かず、対象エリア外の
    // 行が category だけで当たる。本番実測（2026-08-22・Version `3093b95c`）: 同一入力8回で
    // AND 4回 / OR 4回。OR 形は「文化、家族向け、自然」で蔵前の行を旅程の先頭に出した（Issue #174）
    "- area で絞ると決めたら **AND で繋ぐ**こと。category と OR で繋がない ―― `(category IN (...) OR area IN (...))` はエリア外の行が category だけで当たり、絞ったことにならない",
