...
export function createTabiMcpServer(env: Env): McpServer {
  ...
  server.registerTool(
    ...
    {
      title: "集計・抽出",
      // 「意図に合う1件」とは言えない。名指しされた施設が無いときは別の行が返る
      // （Issue #153 の決定・`text-to-sql.ts` の `countRelaxed` doc）。外部クライアントへの
      // 広告で言い切ると、返った行が意図に一致していると誤認させる。
      // **エリアの歯止めも「代表エリアを読み取れたとき」に限る**と書くこと ―― 言い切ると
      // 未知の地名で別エリアの行が返る経路を隠すことになる（cross-model レビュー・信頼度96）
      description:
        "指定したデータセットから、集計意図に近い1件を取り出す。" +
        "意図に名指しされた施設がこのデータセットに無い場合は別の行が返る" +
        "（すり替えたことは応答に現れない）。" +
        "意図に代表エリア（上野・浅草・渋谷）が書かれているときは、すり替え先も同じエリアに限られ、" +
        "そのエリアの行が1件も無ければ unanswered。" +
        "書かれていないときはデータセット全体から選ばれる。" +
