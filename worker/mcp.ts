...
export function createTabiMcpServer(env: Env): McpServer {
  ...
  server.registerTool(
    "aggregate_dataset",
    {
      ...
      // エリアは Issue #152 で実行後に照合するようになった。代表エリアを読み取れた場合は
      // 一致行だけを回答に使い、別エリアや area = NULL の行では埋めない。
      description:
        "指定したデータセットから、集計意図に近い1件を取り出す。" +
        "意図に名指しされた施設がこのデータセットに無い場合は別の行が返る" +
        "（すり替えたことは応答に現れない）。" +
        "意図から代表エリア（上野・浅草・渋谷）を読み取れた場合は、代表エリアと一致する行だけを返す。" +
        "一致する行が無ければ別エリアで埋めず unanswered になる。" +
