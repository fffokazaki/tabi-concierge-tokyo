...
export type SearchDatasetsOutput = { status: "answered"; candidates: DatasetCandidate[] } | Unanswered;
...
export type GetProvenanceOutput = { status: "answered"; sources: ProvenanceSource[] } | Unanswered;
...
/**
 * 入力が仕様を満たさない場合に返す形（HTTP 400）。未定義パスの 404 も同じ形
 * （`{ error: "not_found" }`）。
 *
