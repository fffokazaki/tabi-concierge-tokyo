...
export type SearchDatasetsInput = {
   ...
   * 目的地と明示されたエリア外として扱い、他に代表エリアがあって **`answered` を返す場合は**
   * `out_of_area` の欠損を添える（応答全体が別理由の `unanswered` になる経路では、
   * 「`unanswered` は理由を1つしか運べない」既知の制限どおり報告されない — API.md §3.1）。
   ...
   * 応答全体が `unanswered` になる経路では他の欠損と同様に落ちる（理由を1つしか運べない制限）。
